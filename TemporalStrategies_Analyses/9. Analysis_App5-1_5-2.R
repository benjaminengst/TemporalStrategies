...
# Load the data #
#################
dat <- read.csv2("Data/LegTimeGESTA20200701.csv")
...
# Estimate the negative binomial model (App 5.1) #
##################################################
...
ff <- phase_total ~ independent + election + completed + factor(leadingcom) + factor(lp) 
...
#######################################
pdf("Outcome/App5-2_consent.pdf", width=8, height=7); par(mar=c(1, 8.5 , 1, 0) + 1)
par(mfrow = c(2, 1))
barplot(cbind(
  rbind(mean(dat$phase_total[dat$independent == 1])),
  
  rbind(mean(dat$phase_total[dat$independent == 0])),
...
plot(seq(0:5)
     ...
     , yaxt="n" 
     , ylab=""
     , xlab="Length in frist chamber in days"
     , sub = "" 
...
text(-55, .2, labels = "... potential gain \n (deceleration)", srt = 360,  xpd = TRUE, cex=1)
text(-55, .5, labels = "... neither gain, nor loss", srt = 360,  xpd = TRUE, cex=1)
text(-55, .8, labels = "... potential loss \n (acceleration)", srt = 360, xpd = TRUE, cex=1)
...
########################################
pdf("Outcome/App5-2_objection.pdf", width=8, height=7); par(mar=c(1, 8.5 , 1, 0) + 1)
par(mfrow = c(2, 1))
barplot(cbind(
  rbind(mean(dat$phase_total[dat$independent == 1])),
...
stargazer(nbmodel2, nbmodel4
          ...
          , covariate.labels = c("Potential 2$^{nd}$ Chamber Change"
                                 ...
                                 , "Legislative Periode 9 (=1)"
                                 , "Legislative Periode 10 (=1)"
                                 , "Legislative Periode 11 (=1)"
                                 , "Legislative Periode 12 (=1)"
                                 , "Legislative Periode 13 (=1)"
                                 , "Legislative Periode 14 (=1)"
          ...
          , notes = c("$^{***}$p $<$ .01; $^{**}$p $<$ .05; $^{*}$p $<$ .1 <br> Reference Topic - Transportation, Building and Housing <br> Reference Legislative Periode 8")
          , no.space=FALSE
          , single.row=FALSE
          , out = "Outcome/App5-1.htm")
