...
# Estimate the negative binomial model (App 3.1) #
##################################################
summary(nbmodel1 <- glm.nb(phase_total                            
                           ~ (independent * zustimmg)
                           + independent
                           + zustimmg 
                        , data = dat))
nobs(nbmodel1)
...
# Calcualte expected values and first differences (App.3.3; App 2.2) #
#######################################################################
# If the bill is a consent bill then Zustimmung == 1
# Those in the szenario we need to include: 
# Independent == c(-1,0,1)
# Zustimmung == 1
# Zustimmung X Independent == Zustimmung X Independent

# Set the szenario for Independent
scenario <- c(-1, 0, 1)
# Shape dataset to simulate quantities of interest
ff <- phase_total ~ independent + zustimmg + election + compresOpp + completed + factor(leadingcom) + factor(lp) + (independent * zustimmg) 
...
# Set the szenario for Zustimmung X Independent
scenario <- c(-1, 0, 1)
sel <- which(colnames(mm) == "independent:zustimmg") # select variable of interest
for (i in 1:length(scenario)) {
  dat_sim[, sel, i] <-  scenario[i] 
}

# Set the szenario for Zustimmung == 1
sel <- which(colnames(mm) == "zustimmg") # select variable of interest
for (i in 1:length(scenario)) {
  dat_sim[, sel, i] <-  1 
}
...
# Plot the results in App 3.3 (as PDF) #
########################################
pdf("Outcome/App3-3_consent.pdf", width=8, height=7); par(mar=c(1, 8.5 , 1, 0) + 1)
par(mfrow = c(2, 1))
barplot(cbind(
...
# Set the szenario for Zustimmung X Independent == 0 (for objection)
sel <- which(colnames(mm) == "independent:zustimmg") # select variable of interest
# Put them in the data frames interesting scenarios
for (i in 1:length(scenario)) {
  dat_sim[, sel, i] <-  0
...
# Set the szenario for Zustimmung == 0
sel <- which(colnames(mm) == "zustimmg") # select variable of interest
for (i in 1:length(scenario)) {
  dat_sim[, sel, i] <-  0 
}
...
# Plot the results in App 3.3 (as PDF) #
########################################
pdf("Outcome/App3-3_objection.pdf", width=8, height=7); par(mar=c(1, 8.5 , 1, 0) + 1)
par(mfrow = c(2, 1))
barplot(cbind(
  rbind(mean(dat$phase1[dat$independent == 1])
        , mean(dat$phase2[dat$independent == 1])
...
# Safe all models in table (html) #
###################################
# Table App. 3.2
stargazer(nbmodel1, nbmodel2
          ...
          , column.labels = c("bivariate", "w/controls")
          , covariate.labels = c("Potential 2$^{nd}$ Chamber Change"
                                 ...
                                 , "Pot. 2$^{nd}$ Ch. Change X Consent Bill (=1)"
          )
