...
# Estimate the negative binomial model (App 7.1) #
##################################################
...
# Calcualte expected values and first differences (App 7.2; App 2.2) #
#######################################################################
...
#######################################
pdf("Outcome/App7-2_consent.pdf", width=8, height=7); par(mar=c(1, 8.5 , 1, 0) + 1)
par(mfrow = c(2, 1))
barplot(cbind(
...
########################################
pdf("Outcome/App7-2_objection.pdf", width=8, height=7); par(mar=c(1, 8.5 , 1, 0) + 1)
par(mfrow = c(2, 1))
barplot(cbind(
  rbind(mean(dat$phase1[dat$independent == 1])
        , mean(dat$phase2[dat$independent == 1])
