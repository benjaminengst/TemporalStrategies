...
table(dat$potGain_past, dat$potLoss_past)
dat$independent <- 0
  dat$independent[dat$potGain_past == 1] <- 1
  dat$independent[dat$potLoss_past == 1] <- -1
table(dat$independent)

# CONSENT BILLS (SYMETRIC BICAMERALISM) #
#########################################

# Estimate the negative binomial model (App 4.1 - Model 2a) #
#############################################################
...
# Calcualte expected values and first differences (App 4.2; App 2.2) #
#######################################################################
...
# Plot the results in App 4.3 (as PDF) #
########################################
pdf("Outcome/App4-3_consent.pdf", width=8, height=7); par(mar=c(1, 8.5 , 1, 0) + 1)
par(mfrow = c(2, 1))
barplot(cbind(
  rbind(mean(dat$phase1[dat$independent == 1])
        ...
        , mean(dat$phase5[dat$independent == 1])),
  ...
  rbind(mean(dat$phase1[dat$independent == 0])
        ...
        , mean(dat$phase5[dat$independent == 0])),
...
, las = 1
, col = c(gray(.9), gray(.7), gray(.5), gray(.3), gray(.1))
, border = NA
, xaxt = "n"
, xlim=c(0,200)
, axes = TRUE
...
legend( "bottomright"
        , legend = c("Phase 1"
                     ...
                     ,"Phase 4"
                     ,"Phase 5")
        , fill = c(gray(.9), gray(.7), gray(.5), gray(.3), gray(.1))
        , border = FALSE
        #    , lwd = 1 
...
# Estimate the negative binomial model (App 4.1 - Model 2b) #
#############################################################
...
# Plot the results in App 4.3 (as PDF) #
########################################
pdf("Outcome/App4-3_objection.pdf", width=8, height=7); par(mar=c(1, 8.5 , 1, 0) + 1)
par(mfrow = c(2, 1))
barplot(cbind(
  rbind(mean(dat$phase1[dat$independent == 1])
        , mean(dat$phase2[dat$independent == 1])
...
####################################
# Table App. 4.1 - Model 2a / Model 2b
stargazer(nbmodel2, nbmodel4
          ...
          , no.space=FALSE
          , single.row=FALSE
          , out = "Outcome/App4-1_Models2.htm")
